...
const fetchCompany = async () => {
    try {
        const response = await get(`/companys/${props.companyId}`);
...
const handleEdit = () => {
    router.visit(route('companys.edit', props.companyId));
};

const handleDelete = async () => {
    try {
        await destroy(`/companys/${props.companyId}`);
        router.visit(route('companys.index'));
    } catch (error) {
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold companying-tight text-gray-800">
                    Company Details
                </h2>
                            ...
                            <div class="flex items-start justify-between">
                                <div>
                                    ...
                                    <div class="mt-2 flex items-center gap-2">
                                        <span :class="['inline-flex rounded-full px-3 py-1 text-sm font-semibold', getStatusColor(company.status)]">
                                            {{ company.status }}
                                        </span>
                            ...
                            <dl class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                ...
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <dd class="mt-1 text-sm text-gray-900">
                                            {{ company.assigned_to?.name || 'Unassigned' }}
                                        </dd>
                                    </div>
                                </div>
                            </dl>
